...
router.patch('/:id', authenticate, authorize('ADMIN', 'RECEPTIONIST', 'DOCTOR'), validateBody(updateAppointmentStatusSchema), asyncHandler(async (req: AuthRequest, res: Response) => {
  const appt = await appointmentsService.update(req.params.id, req.body);
  res.json(appt);
}));

router.delete('/:id', authenticate, authorize('ADMIN', 'RECEPTIONIST'), asyncHandler(async (req: AuthRequest, res: Response) => {
